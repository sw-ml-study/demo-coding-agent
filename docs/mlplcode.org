...
#+startup: showall
#+property: header-args:mlpl :results output :exports both
#+options: toc:2 num:t
...
To evaluate a block in place with =C-c C-c=, load sw-MLPL's org-babel
backend from the adjacent checkout. Evaluate this block once (it is not
tangled):

#+begin_src emacs-lisp :tangle no
(add-to-list 'load-path (expand-file-name "../../sw-mlpl/elisp" (file-name-directory (buffer-file-name))))
(require 'ob-mlpl)
(setq org-babel-mlpl-command (expand-file-name "../../sw-mlpl/target/release/mlpl-repl" (file-name-directory (buffer-file-name))))
#+end_src

A block that defines functions evaluates silently; the blocks worth running
are the small pure ones, and the entries need arguments, so run those from
the shell with ~just loop~, ~just replay~, or ~just v0~.

To regenerate the sources from this file: ~just tangle~.
